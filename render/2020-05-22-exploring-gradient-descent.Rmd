...
ggplot(sim_d, aes(x, y)) +
  ...
  geom_abline(intercept = 4, slope = 3, 
              color = "#0abeff") +
  geom_abline(intercept = 3, slope = 2, color = "#94e2ff") +
  geom_abline(intercept = 7, slope = 1, color = "#004761")
```

We can calculate the mean square error for each of these lines. The mean square error for the <span style="color:#94e2ff">light blue line</span> is `r round(mse(x, y, 3, 2), 2)`, the <span style="color:#0abeff">slightly darker blue line is </span> `r round(mse(x, y, 4, 3), 2)`, and the <span style="color:#004761">darkest blue line is </span> `r round(mse(x, y, 7, 1), 2)`. So among these three candidate lines, the one that is closest to the observed values, on average, is the darkest line.
